...
CREATE OR REFRESH MATERIALIZED VIEW gold_state_summary AS
SELECT
  state,
...
FROM LIVE.gold_county_day_features
GROUP BY state;

-- COMMAND ----------

CREATE OR REFRESH MATERIALIZED VIEW gold_county_rankings AS
SELECT
  state,
  fips_code,
  county,
...
FROM LIVE.gold_county_risk_scores
